...
function isThumbnailFolder(options, name){
	for(var attributename in options.imageVersions){
    ...
    UploadHandler.prototype.get = function () {
        ...
        fs.readdir(options.uploadDir(), _.bind(function (err, list) {
            _.each(list, function (name) {
                ...
                if (stats.isDirectory() && !isThumbnailFolder(options, name)) {
                	fileInfo = new FileInfo({
                        name: name,
                        size: stats.size,
                        lastModifyDate: stats.mtime,
    ...
    UploadHandler.prototype.destroy = function () {
        ...
        if (fileName.indexOf('.') != -1) {
        	fs.unlink(options.uploadDir() + '/' + fileName, function (ex) {
           	   console.log('delete file ' + options.uploadDir() + '/' + fileName + ex);
               _.each(options.imageVersions, function (value, version) {
        	...
        	deleteFolderRecursive(options.uploadDir());
        	self.emit('delete', options.uploadDir());
